...
class BaseWorkflow(AbstractWorkflow):
    ...
    def connect(self, srcpath: str, destpath: str, *, inpath: str = None):
        ...
        if not obj_dest.events_to_delete:
            # Use existing object from delete_events_list if available
            if obj_dest.delete_events_list:
                obj_dest.events_to_delete.append(obj_dest.delete_events_list[0])
            else:
                # Create a new one if neither list has an item
                obj_dest.events_to_delete.append(DeleteEvents(events=[]))
        
        # Now it's safe to append to the last element
...
class Workflow(BaseWorkflow):
    ...
    def get_jobs(environment: Environment, server: str = "*", folder: str = "*") -> "Workflow":
        ...
        try:
            ...
            print(f"[ERROR] Failed to retrieve job definitions: {error}")
            return None